...
public class DataManager {
  ...
  public static List<AssiciationRepresentive> getAssiciationRepresentivesFromAccounts(){
    List<AssiciationRepresentive> assiciationRepresentives=new LinkedList<>();
    List<Account> accounts=DataManager.getAccounts();
    if(accounts.size()==0) return null;
    for(Account account: DataManager.getAccounts()){
      for(Role role:account.getRoles()){
        if(role instanceof AssiciationRepresentive){
          assiciationRepresentives.add((AssiciationRepresentive) role);
          break;
        }
      }
    }

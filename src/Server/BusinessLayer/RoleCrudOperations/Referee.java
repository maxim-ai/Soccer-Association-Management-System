...
public class Referee extends Role implements Serializable
{
  ...
  public Referee(String aTraining, String aName)
  {
    super(aName);
    training = aTraining;
    //leagues = new ArrayList<Server.BusinessLayer.OtherCrudOperations.League>();
    leagues = new HashMap<>();
  ...
  public boolean updateDetails(String name){
    ...
    super.setName(name);
    Logger.getInstance().writeNewLine("Server.BusinessLayer.RoleCrudOperations.Referee "+before+" update name to: "+super.getName());
    return true;
  }
  /*
  ...
  public void displayAllMatches() throws Exception {
    if (!matchs.isEmpty()) {
      ...
      Logger.getInstance().writeNewLine("Server.BusinessLayer.RoleCrudOperations.Referee "+super.getName()+" watch all his Matches");
    }
    else{
...
//        wasUpdate=true;
//        Logger.getInstance().writeNewLine("Server.BusinessLayer.RoleCrudOperations.Referee "+super.getName()+" update event during the match between: "+match.getHomeTeam().getName()+","+match.getAwayTeam().getName()+" to "+event.getType());
//      }
//      else {
...
//          wasEdit = true;
//          Logger.getInstance().writeNewLine("Server.BusinessLayer.RoleCrudOperations.Referee " + super.getName() + " edit event after the match between: " + match.getHomeTeam().getName() + "," + match.getAwayTeam().getName() + " to " + aType);
//
//        }
//        else{
